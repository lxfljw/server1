...
_private.prepareError = function prepareError(err, req, res, next) {
    ...
    //console.log('error-handler.js   prepareError',err);
    console.log(err);
    ...
    if (!(err instanceof errors.WebServiceError)) {
        ...
        if (err.statusCode && err.statusCode === 404) {
            ...
            err = new errors.WebServiceError({
                err: err,
...
_private.JSONErrorRenderer = function JSONErrorRenderer(err, req, res, /*jshint unused:false */ next) {
    ...
    //console.log('error-handler.js   JSONErrorRenderer  ');
    res.json({
        errors: [{
            message: err.message,
...
/*
errorHandler.handleJSONResponse2 = function errorHandler(err, req, res, next) {
    if (_.isArray(err)) {
        err = err[0];
    }
